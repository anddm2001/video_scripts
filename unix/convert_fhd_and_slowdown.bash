...
for f in "$IN"/*.$EXTENSION; do
    ...
    # Выполняем команду ffmpeg для изменения разрешения файла до HD
    # При этом аудио дорожка и кодек останутся без изменений
    ffmpeg -hide_banner -i "$f" -vf "scale=1920:1080,setpts=2.0*PTS" -c:a copy -c:v "$VIDEO_CODEC" "$exportFile"
