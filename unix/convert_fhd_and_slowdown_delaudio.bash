...
for f in "$IN"/*.$EXTENSION; do
    ...
    # Выполняем команду ffmpeg для изменения разрешения файла до FHD + удаление аудиопотока
    ffmpeg -hide_banner -i "$f" -vf "scale=1920:1080,setpts=2.0*PTS" -an -c:v "$VIDEO_CODEC" "$exportFile"
